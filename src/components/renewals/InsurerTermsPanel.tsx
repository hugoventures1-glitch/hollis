      ...
      {showForm && (
        <div className="rounded-lg bg-background border border-border p-4 space-y-3">
          ...
          <input
            ...
            className="block w-full text-[13px] bg-surface border border-border rounded-lg px-3 py-2 text-text-primary placeholder-text-tertiary focus:outline-none focus:border-border"
          />
          <textarea
            value={rawText}
            ...
            rows={6}
            className="block w-full text-[13px] bg-surface border border-border rounded-lg px-3 py-2 text-text-primary placeholder-text-tertiary focus:outline-none focus:border-border resize-none font-mono"
          />
          <div className="flex gap-2">
            <button
              onClick={handleAdd}
              disabled={isPending || !rawText.trim() || !insurerName.trim()}
              className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 rounded-lg bg-hover-overlay text-text-primary hover:bg-hover-overlay transition-colors disabled:opacity-40"
            >
              {isPending ? (
                <>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="text-[14px] font-semibold text-text-primary">{term.insurer_name}</div>
                    {term.effective_date && (
                ...
                <div className="flex items-baseline gap-3">
                  <div>
                    <div className="text-[11px] text-text-tertiary uppercase tracking-wider">Premium</div>
                    ...
                    {isExpanded && (
                      <div className="mt-2 space-y-2">
                        {term.new_exclusions.length > 0 && (
                          <div>
                            <div className="text-[11px] font-medium text-text-secondary mb-1">New Exclusions</div>
                ...
                {/* Notes */}
                {term.notes && (
                  <div className="text-[12px] text-text-secondary italic border-t border-border pt-2">
                    {term.notes}
                  </div>
                )}
