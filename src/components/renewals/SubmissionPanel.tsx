...
export function SubmissionPanel({ policyId, hasTerms }: SubmissionPanelProps) {
        ...
        <div className="rounded-lg bg-[#FAFAFA]/[0.06] border border-[#1C1C1C] px-4 py-4 space-y-2">
          ...
          <ul className="space-y-1">
            {sentAddresses.map((addr) => (
              <li key={addr} className="text-[12px] text-[#8a8a8a]">→ {addr}</li>
            ))}
          </ul>
        </div>
        <button
          onClick={() => { setSent(false); setEmails([""]); setSentAddresses([]); }}
          className="text-[12px] text-[#8a8a8a] hover:text-[#8a8a8a] transition-colors"
        >
  ...
  return (
    <div className="rounded-xl bg-[#111111] border border-[#1C1C1C] p-5 space-y-4">
      ...
      {!hasTerms && (
        <div className="rounded-lg bg-[#1C1C1C] border border-[#1C1C1C] px-4 py-3 text-[12px] text-[#9e9e9e]">
      ...
      <div className="space-y-2">
        ...
        {emails.map((email, i) => (
          <div key={i} className="flex items-center gap-2">
            <input
              ...
              disabled={isPending}
              className="flex-1 text-[13px] bg-[#0C0C0C] border border-[#2a2a35] rounded-lg px-3 py-2 text-[#FAFAFA] placeholder-[#6b6b6b] focus:outline-none focus:border-[#555555] disabled:opacity-50"
            />
            {emails.length > 1 && (
              <button
                onClick={() => removeEmail(i)}
                disabled={isPending}
                className="p-1.5 text-[#6b6b6b] hover:text-[#8a8a8a] transition-colors disabled:opacity-50"
              >
                <X size={13} />
              </button>
            )}
        ...
        <button
          onClick={addEmail}
          disabled={isPending}
          className="flex items-center gap-1.5 text-[12px] text-[#8a8a8a] hover:text-[#8a8a8a] transition-colors disabled:opacity-50"
        >
          <Plus size={12} />
          Add another insurer
        </button>
