...
export function SubmissionPanel({ policyId, hasTerms }: SubmissionPanelProps) {
  ...
  if (sent) {
    return (
      <div className="rounded-xl bg-surface border border-border p-5 space-y-3">
        ...
        <div className="rounded-lg bg-hover-overlay border border-border px-4 py-4 space-y-2">
          ...
          <ul className="space-y-1">
            {sentAddresses.map((addr) => (
              <li key={addr} className="text-[12px] text-text-secondary">→ {addr}</li>
            ))}
          </ul>
        </div>
        <button
  ...
  return (
    <div className="rounded-xl bg-surface border border-border p-5 space-y-4">
      ...
      {!hasTerms && (
        <div className="rounded-lg bg-border border border-border px-4 py-3 text-[12px] text-text-secondary">
      ...
      <div className="space-y-2">
        ...
        {emails.map((email, i) => (
          <div key={i} className="flex items-center gap-2">
            <input
              ...
              disabled={isPending}
              className="flex-1 text-[13px] bg-background border border-border rounded-lg px-3 py-2 text-text-primary placeholder-text-tertiary focus:outline-none focus:border-border disabled:opacity-50"
            />
            {emails.length > 1 && (
              <button
        ...
        <button
          onClick={addEmail}
          disabled={isPending}
          className="flex items-center gap-1.5 text-[12px] text-text-secondary hover:text-text-primary transition-colors disabled:opacity-50"
        >
          <Plus size={12} />
          Add another insurer
        </button>
